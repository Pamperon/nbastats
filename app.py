# app.py — NBA Stats + Bet365 Extractor (veloce: cache senza spinner, dual gamelogs, bottone calcolo)
# - show_spinner=False su tutte le cache (niente "Running ...")
# - get_dual_gamelogs() per evitare chiamate duplicate
# - cache locale nel batch
# - bottone "Calcola / Aggiorna" nel tab singolo
# - tuning HTTP per nba_api
# - import lazy di nba_api e BeautifulSoup (fallback parser)
...
# —— Tuning HTTP nba_api (timeout + UA) ——
if HAS_NBA:
    import importlib
    try:
        http_mod = importlib.import_module("nba_api.stats.library.http")
        NBAStatsHTTP = http_mod.NBAStatsHTTP
        NBAStatsHTTP.timeout = 6  # default più alto; abbassiamo per sveltire fallimenti
...
# —— Dual gamelog (corrente + precedente già filtrati) ——
...
def get_last_n_games_from_dual(cur: pd.DataFrame, prev: pd.DataFrame, n: int) -> pd.DataFrame:
    combo = pd.concat([cur, prev], ignore_index=True).sort_values("GAME_DATE", ascending=False)
    return combo.head(n)
...
with tab_batch:
    ...
    if uploaded:
        ...
        # Cache locale per riusare i gamelog in questo run
        ...
        for i, row in df_in.iterrows():
            ...
            # Ultime 5/10 cross-stagione senza ricallare l'API
            prev_gl = get_gl(pid, prev_season(CURRENT_SEASON))
            last5  = get_last_n_games_from_dual(filter_game_type(glog, "Totale"),
                                                filter_game_type(prev_gl, "Totale"), 5)
            last10 = get_last_n_games_from_dual(filter_game_type(glog, "Totale"),
                                                filter_game_type(prev_gl, "Totale"), 10)
...
with tab_single:
    ...
    if q.strip():
        ...
        if not matches:
            ...
            # 👉 Calcolo esplicito per evitare rerun continui
            run_calc = st.button("🔁 Calcola / Aggiorna", type="primary", use_container_width=True)
            if not run_calc:
                st.info("Premi **Calcola / Aggiorna** per caricare i dati e il grafico.")
                st.stop()

            # Dati: una sola chiamata per corrente+precedente già filtrate
            try:
            ...
            if chart_range == "Ultime 5":
                dplot = get_last_n_games_from_dual(df_cur, df_prev, 5)
                title = f"{sel['full_name']} | Ultime 5 — {m}"
            elif chart_range == "Ultime 10":
                dplot = get_last_n_games_from_dual(df_cur, df_prev, 10)
            ...
            last5  = get_last_n_games_from_dual(df_cur, df_prev, 5)
            last10 = get_last_n_games_from_dual(df_cur, df_prev, 10)
